...
│ .totalfitur`;
    ...
    } else if ([".gpt4", "gpt4", ".gpt", "gpt", ".gemini", "gemini", ".deepseek", "deepseek", ".ai", "ai", ".bing", "bing", ".askai", "askai", ".ask", "ask"].includes(body.split(" ")[0].toLowerCase())) {
      ...
      try {
        ...
        if (!answer) {
            try {
               // Gunakan Endpoint Pollinations POST yang baru tanpa spesifikasi model agar gratis
               const payload = {
                   messages: [
            ...
            if (!answer) {
                try {
                    // Fallback menggunakan GET request yang simpel
                    const combinedQuery = `${systemPrompt}\n\nPertanyaan: ${query}`;
                    const textRes = await axios.get('https://text.pollinations.ai/'+encodeURIComponent(combinedQuery), {
                        timeout: 15000,
                    ...
                    console.error("Pollinations GET error:", e2.message);
                }
            }
            
            if (!answer) {
                if (!process.env.GEMINI_API_KEY) {
                    answer = "❌ *Gagal mendapatkan respon AI.*\n\nKarena sistem AI publik gratis sedang sibuk, disarankan menambahkan `GEMINI_API_KEY` di Dashboard Server (seperti Railway) agar fitur AI berfungsi stabil tanpa limit.\n\n*Cara mendapatkan API Key gratis:*\n1. Buka https://aistudio.google.com/app/apikey\n2. Buat API Key baru\n3. Copy dan masukkan sebagai variable `GEMINI_API_KEY` di Server.";
                } else {
                    answer = `❌ *Gagal mendapatkan respon AI.*\n\nAPI Key Gemini kamu terdeteksi, namun terjadi error saat menghubungi Google: _${geminiError}_` + "\n\nPastikan API Key kamu benar, masih aktif, dan memiliki kuota di Google AI Studio. Server fallback cadangan juga sedang penuh.";
                }
            }
        }
      ...
      try {
          ...
          if (removeBgApiKey) {
              ...
              // Fallback to local background removal AI
              await this.sock.sendMessage(jid, { text: "⏳ *Sedang memproses (mode AI Lokal tanpa API Key)...*\n\nMungkin memakan waktu sedikit lebih lama. Tambahkan REMOVEBG_API_KEY di server jika ingin proses lebih cepat." }, { quoted: msg });
              const { removeBackground } = require('@imgly/background-removal-node');
              const blob = new Blob([media], { type: 'image/jpeg' });
              const resultBlob = await removeBackground(blob);
              buffer = Buffer.from(await resultBlob.arrayBuffer());
          }
         ...
         await this.sock.sendMessage(jid, { text: "❌ *Gagal memproses gambar.*\n\nJika menggunakan mode AI lokal, pastikan memori server cukup (minimum 1GB RAM) atau gunakan API Key gratis dari remove.bg dan pasang `REMOVEBG_API_KEY`." }, { quoted: msg });
         await this.sock.sendMessage(jid, { react: { text: "❌", key: msg.key } });
      }
...
│ .losmenmelati`;
      ...
      if (body.includes("on")) {
       ...
       // Try extracting custom text
       const customText = messageContent.replace(/^\.?iqc\s*/i, "").trim();
       const text = customText || (quotedMsg ? (quotedMsg.conversation || quotedMsg.extendedTextMessage?.text) : "") || "Teks\nIsi sendiri..";

       try {
           await this.sock.sendMessage(jid, { text: "⏳ *Sedang memproses iqc (iPhone Quote Center)...*" }, { quoted: msg });
           ...
           const startY = 80;
           const reactionY = startY;
           const reactionHeight = 50;
           const bubbleY = reactionY + reactionHeight + 10;
           const menuY = bubbleY + bubbleHeight + 10;
           const menuHeight = 350;
           
           const width = 500;
           const height = menuY + menuHeight + 80;
           ...
           const watermark = `@${this.customBotName || "SAHRIL"} [IQC]`;
...
<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- Background (WhatsApp Chat Background Color) -->
  <rect width="100%" height="100%" fill="#efe6dd" />

  <!-- Reaction Bar -->
  <rect x="30" y="${reactionY}" width="380" height="50" rx="25" fill="#ffffff" filter="url(#shadow)" />
  <text x="45" y="${reactionY + 34}" font-size="24">👍 ❤️ 😂 😮 😢 🙏 🤣</text>
  <circle cx="380" cy="${reactionY + 25}" r="14" fill="#f0f2f5" />
  <path d="M374 ${reactionY + 25} H386 M380 ${reactionY + 19} V${reactionY + 31}" stroke="#54656f" stroke-width="2" stroke-linecap="round" />

  <!-- Chat Bubble -->
  <path d="M30 ${bubbleY} L15 ${bubbleY} L30 ${bubbleY + 15} Z" fill="#ffffff" filter="url(#shadow)" />
  ...
  ${generateTextPath(watermark, {x: 30, y: menuY + menuHeight + 35, fontSize: 14, fill: '#667781'})}
           ...
           await this.sock.sendMessage(jid, { image: finalBuffer, caption: "Ini IQC mu!" }, { quoted: msg });
       } catch (e) {
